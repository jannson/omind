...
class _node_root(_node_base):
    ...
    def back(self, ctx, xml_root, xml_node, root):
        ...
        ctx.obj_string += str_obj + '.id = "' + self.id[3:] + '";\n';
        ctx.obj_string += str_obj + '.text = "' + self.text + '";\n';
        str = ''
...
def xml_leaf_first(ctx, map):
    root = map.getchildren()[0]
    ctx.title = root.attrib["TEXT"]
    ctx.ns.push(map, root, None, ctx.the_root, True);
